...
`<env>` 只能是 `uat` 或 `prod`。禁止使用 `kv/shared/*` 保存环境凭据；真正共享
...
Terraform 输出 WIF provider、service account、project ID 和 region；Ansible 在
部署 Vault 后将这些值写入对应环境的 `platform/oidc`、`platform/gcp` 路径。
Vault policy 和 auth role 由 Ansible 管理，Terraform 不直接写入 secret value。
