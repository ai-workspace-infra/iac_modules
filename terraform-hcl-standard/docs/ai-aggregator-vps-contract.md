...
Each provider adapter must expose the same runtime facts to CMDB:
...
60-minute cloud-init TTL guard. Vultr and GCP adapters provide the same
declarative shape for future CPA/Gateway placement. Production nodes remain
external persistent hosts and are not imported into Terraform state by v1.
