...
- 资源声明放共享 `${GITOPS_ROOT}/resources/svc.plus/uat/vultr/ai-workspace.yaml`，全局段经 `terraform.auto.tfvars.json` 传给 `variables.tf`。
...
```
vultr-vps/
  ${GITOPS_ROOT}/resources/svc.plus/uat/vultr/ai-workspace.yaml   # 声明：唯一人工入口 / CMDB 源
...
> 共享 `templates/`。新增一套主机只需加一个 `${GITOPS_ROOT}/resources/svc.plus/<env>/vultr/<name>.yaml`
...
| 文件 | 角色 |
|------|------|
| `../../${GITOPS_ROOT}/resources/svc.plus/uat/vultr/ai-workspace.yaml` | **唯一人工入口**：global / ssh_keys / hosts 资源声明 (CMDB 源) |
...
# 编辑 ${GITOPS_ROOT}/resources/svc.plus/uat/vultr/ai-workspace.yaml（填真实 ssh 公钥 / 调整主机），然后：
...
> 多套主机：加 `${GITOPS_ROOT}/resources/svc.plus/<env>/vultr/<name>.yaml` + 一个 workdir，复用同一 scripts：
> `scripts/generate.py render --resources ${GITOPS_ROOT}/resources/svc.plus/<env>/vultr/<name>.yaml --workdir envs/<name>`

## 调整主机

改 `../../${GITOPS_ROOT}/resources/svc.plus/uat/vultr/ai-workspace.yaml` 即可增删机器或改套餐/区域/分组/host_vars，重跑 `scripts/generate.py render`。
